...
import { Fa500Px, FaPrint } from 'react-icons/fa';
...
const AnnualTraining = () => {
  ...
  monthsInData.forEach((month, index) => {
    ...
    // Convert set to sorted array with custom sorting to handle year wrap-around
    dynamicWeeksByMonth[month] = Array.from(weeks).sort((a, b) => {
      // If weeks span year end, put higher weeks before lower weeks (e.g., 49-52 before 1)
      if (a < 10 && b > 40) {
        return 1; // a after b
      }
      if (a > 40 && b < 10) {
...
<div className="bg-sky-400 text-white p-2 flex justify-between rounded-t-lg">
      ...
      <div className="mb-4 mt-2 flex justify-between items-center space-x-4">
        ...
        <div className='mx-2'>
          ...
          <select
            ...
            className="p-2 border border-gray-300 rounded-lg"
          >
            <option value="IATF">IATF (International Automotive Task Force)</option>
      ...
      <div >
        <button
          onClick={() => {
            const printContents = document.getElementById('annual-training-table').outerHTML;
            const printWindow = window.open('', '', 'width=800,height=600');
            printWindow.document.write('<html><head><title>Print Calendar</title>');
            printWindow.document.write('<style>@page { size: landscape; } table {width: 100%; border-collapse: collapse;} td, th {border: 1px solid #ccc; padding: 8px;}</style>');
          ...
          className="px-4 py-2 bg-gray-600 text-white rounded hover:bg-sky-900 focus:outline-none focus:ring-2 focus:ring-gray-400"
        >
          <FaPrint/>
        </button>
      </div>
      ...
      {!loading && !error && trainingData.length > 0 && (
        <>
          <table id="annual-training-table" className="min-w-full border border-gray-300 table-fixed">
            <tbody>
              <tr>
                <td   className="border border-gray-300 px-2 py-1 font-semibold w-12 h-12 bg-blue-100 text-center align-middle whitespace-nowrap" colSpan={6}>Annual Training Calendar for {trainingName}</td>
              </tr>
              <tr>
                <td
