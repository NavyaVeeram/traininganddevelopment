...
const generatePDF = async () => {
  ...
  //  titleDiv.textContent = `Annual Training Calendar for ${trainingType}`;
...
legendContainer.style.right = "2";
legendContainer.style.top = "0";
// legendContainer.style.paddingTop = "8px";
// legendContainer.style.paddingRight = "8px";
// Only add Special Position legend if not IATF
...
if (trainingType !== "IATF") {
  ...
  specialSpan.innerHTML = "● Special Position";
  specialSpan.style.color = "#16a34a";
  specialSpan.style.fontWeight = "600";
  legendContainer.appendChild(specialSpan);  // Append inside the if block
}

// Additional legend
const additionalSpan = document.createElement("span");
additionalSpan.innerHTML = "● Additional";
additionalSpan.style.color = "#2563eb";
additionalSpan.style.fontWeight = "600";

// External legend
const externalSpan = document.createElement("span");
externalSpan.innerHTML = "● External";
externalSpan.style.color = "#9333ea";
externalSpan.style.fontWeight = "600";

// Append the other two legends
legendContainer.appendChild(additionalSpan);
legendContainer.appendChild(externalSpan);

    titleContainer.appendChild(titleDiv);
    titleContainer.appendChild(legendContainer);
...
// Add header cells for each month in the chunk

  const headerCell = document.createElement("td");
  headerCell.textContent = "Months";
...
foundryCell.textContent = "FD-Foundry / MG-Machining Group / GMO - GM Office / FA - Facility / GA - General Affairs / QA - Quality Assurance / BU - Business / PMC - Production & Material Control / HSE - Health Safety Environment / HR - Human Resources / IT - Information Technology / PU - Purchase / WH - Warehouse / FI - Finance";
...
foundryCell.style.paddingTop = "4px";      // Exact top padding [web:29][web:33]
foundryCell.style.paddingBottom = "8px";   // Exact bottom padding [web:29][web:33]
foundryCell.style.paddingLeft = "10px";    // Left padding
foundryCell.style.paddingRight = "10px";   // Right padding
foundryCell.style.verticalAlign = "top";   // Aligns content to top edge [web:21]
foundryCell.style.lineHeight = "1.4";      // Even line spacing
...
foundryCell.style.fontFamily = "Arial, sans-serif";
    ...
    const footerText =
      trainingType === "HSE"
        ? 'We are following "IS014001 & IS045001 CAPD Method 10.3 Continuous Improvement Spirit to improve our GTI"'
    ...
    // Table Image
    doc.addImage(imgData, "PNG", 10, 22, 277, 150);
    ...
    // Add "Foundry FD" div on the last page only
    // if (pageIndex === monthsInData.length - monthsPerPage || 
    //     pageIndex + monthsPerPage >= monthsInData.length) {
    //   doc.setFontSize(10);
    //   doc.setFont("helvetica", "bold");
    //   doc.text("Foundry FD", doc.internal.pageSize.getWidth() / 2, boxY - 5, {
    //     align: "center",
    //   });
    // }
