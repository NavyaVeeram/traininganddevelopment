  ...
  const handleUpdate = async () => {
    try {
      ...
      const updatedDataWithCreatedBy = {
        ...
        Persons:
          editingData.Persons !== undefined && editingData.Persons !== null
            ? String(editingData.Persons)
            : null,
        No_Hrs:
          editingData.No_Hrs !== undefined && editingData.No_Hrs !== null
            ? parseInt(editingData.No_Hrs, 10)
            : null,
        Evaluation_Period: editingData.Evaluation_Period
          ? parseInt(editingData.Evaluation_Period, 10)
          : null,
      };

      const res = await fetch(
        `/api/update_approval_form?Program_Id=${programId}`,
        {
          method: "POST",
            ...
            <div className="flex items-center gap-2 text-sm">
              <div>
                {accessRole !== "Res_Person" &&
                  accessRole !== "HOS" &&
                  accessRole !== "HOD" && <MonthlyCalender />}
              </div>
              ...
              <table className="w-full border-collapse text-sm">
                <thead className="bg-gray-100">
                  <tr>
                    ...
                    <th
                      className="border p-2 cursor-pointer text-left"
                      onClick={() => handleSort("Train_Purpose")}
                    >
                      Purpose{" "}
