...
const WeeklyTrainingComponent = ({ selectedYear }) => {
  ...
  useEffect(() => {
    const fetchTrainingData = async () => {
      ...
      try {
        // Fetch IATF data
        const iatfResponse = await fetch(`/api/get_curr_next_week_prog_iatf?year=${selectedYear}`);
        if (!iatfResponse.ok) {
          throw new Error('Failed to fetch IATF training data');
        }
        const iatfResult = await iatfResponse.json();
        setIatfData(iatfResult);

        // Fetch HSE data
        const hseResponse = await fetch(`/api/get_curr_next_week_prog_hse?year=${selectedYear}`);
        if (!hseResponse.ok) {
          throw new Error('Failed to fetch HSE training data');
        }
        const hseResult = await hseResponse.json();
        setHseData(hseResult);
      } catch (err) {
  ...
  const currentWeekIatf = iatfData.filter(item => item.Week === currentWeek);
  const nextWeekIatf = iatfData.filter(item => item.Week === nextWeek);
  const currentWeekHse = hseData.filter(item => item.Week === currentWeek);
  const nextWeekHse = hseData.filter(item => item.Week === nextWeek);
