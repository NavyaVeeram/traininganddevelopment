...
const RatingDistributionPage = () => {
  ...
  useEffect(() => {
    const fetchRatingDistribution = async () => {
      try {
        if (!selectedYear || !selectedTrainingName || !selectedProgramName) {
          // Do not set error or fetch data if filters are incomplete
          setData([]);
          setError(null);
          setLoading(false);
          return;
        ...
        if (selectedYear) params.append("Year_No", selectedYear.getFullYear());
        if (selectedTrainingName) params.append("Training_Name", selectedTrainingName.value);
        if (selectedProgramName) params.append("Program_Id", selectedProgramName.value);
      ...
      if (selectedYear) {
        try {
          ...
          const response = await fetch(
            `/api/get_program_name_dropdown?Year_No=${selectedYear.getFullYear()}&Training_Name=${trainingName}`
          );
          if (!response.ok) {
          ...
          // If selectedProgramName is null or not in the new list, reset or set it
          if (selectedProgramName) {
            ...
            // Do not auto-select the first program name when selectedProgramName is null
            // setSelectedProgramName({ value: result[0].id, label: result[0].name });
          }
        } catch (err) {
          console.error(err);
          setProgramNames([]);
  ...
  useEffect(() => {
    ...
    const fetchInitialProgramNames = async () => {
      try {
        const response = await fetch(
          `/api/get_program_name_dropdown?Year_No=${selectedYear ? selectedYear.getFullYear() : new Date().getFullYear()}&Training_Name=${selectedTrainingName ? selectedTrainingName.value : 'IATF'}`
        );
        if (!response.ok) {
          throw new Error("Failed to fetch program names");
        }
  ...
  useEffect(() => {
    async function fetchRatingCounts() {
      try {
        ...
        const params = new URLSearchParams();
        params.append("Year_No", selectedYear.getFullYear());
  ...
  return (
     <div className="max-w-full mx-auto bg-white p-2 w-full">
      ...
      {/* Removed prior Program_Name display table as per user request */}
          ...
          <div className="flex mt-3 items-center space-x-1">
            ...
            <Select
              ...
              onChange={(newValue) => {
                ...
                setSelectedProgramName(null); // Reset Program_Name dropdown when Training_Name changes
              }}
