...
const TetReportsforTL = () => {
  ...
  useEffect(() => {
    const fetchProgramName = async () => {
      try {
        const res = await fetch(`/api/get_tet_form_program_name?id=${programId}`);
        if (!res.ok) {
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    if (!res.ok) {
                      ...
                      throw new Error(errorData.error || 'Failed to update Res_Person');
                    }
                    alert('Res_Person updated successfully.');
                    setShowPopup(false);
                    setDropdownValue(null);
                    setUpdatedEmployees(prev => [...new Set([...prev, ...selectedEmployees])]);
                  } catch (error) {
