...
export default async function handler(req, res) {
  if (req.method === "GET") {
    const { program_id,Training_Name } = req.query;

if (!program_id || !Training_Name) {
  return res.status(400).json({ error: "Program ID and Training Name are required" });
}

    try {
      // Call the stored procedure to get the training entry details by Program_Id
      const trainingDetails = await prisma.$queryRaw`
        EXEC dbo.Get_Training_Att_Entry @Program_Id = ${program_id},@Training_Name = ${Training_Name}
      `;
